...
NOTIFY(_entity,"Gunship is on the way and will signal on arrival.")
...
private _height = (AGLToASL _position # 2) + 800;
...
[{
	...
	if (!alive _vehicle) exitwith {
		...
		[_entity,false] call SSS_fnc_updateMarker;
		_entity setVariable ["SSS_cooldown",_entity getVariable "SSS_cooldownDefault",true];
		[_entity,"Rearmed and ready for further tasking."] call SSS_fnc_cooldown;
	};
	NOTIFY(_entity,"Gunship is in the requested area. (Connect via interaction menu)")
	...
	[{
		...
		NOTIFY_1(_entity,"Gunship is leaving the area. Gunship support will be available again in %1.",PROPER_COOLDOWN(_entity))
