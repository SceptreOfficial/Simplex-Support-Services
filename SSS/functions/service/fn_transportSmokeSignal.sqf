...
[{
	...
	private _signalSeen = ["an IR signal",format ["a %1 smoke",_signal call SSS_fnc_getSmokeColor]] select (_signalType == "SmokeShell");
	...
	[{WAIT_CONDITION_BASE || !(_vehicle getVariable "SSS_needConfirmation")},{
		...
		if !(_vehicle getVariable "SSS_signalApproved") exitWith {
			...
			[_vehicle,_position,false] call SSS_fnc_transportSmokeSignal;
		};
		...
		[_vehicle,_signalPos,0,"MOVE","","","","",WP_DONE] call SSS_fnc_addWaypoint;
