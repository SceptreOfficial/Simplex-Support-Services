...
if (!isNull (findDisplay 312)) then {
	...
	private _fnc_selectionUI = {
		...
		[_confirm,"ButtonClick",{
			...
			[_requesters,_entities] call FUNC(unassignRequesters);

			_display closeDisplay 0;
			ZEUS_MESSAGE("Requesters unassigned");
	...
	if (_object isKindOf "CAManBase") then {
		...
		[_fnc_selectionUI] call FUNC(zeusSelection);
	};

	deleteVehicle _logic;
} else {
	[{
		((synchronizedObjects _this) select {_x isKindOf "SSS_Module_Base"}) isEqualTo []
	},{
		deleteVehicle _this;
