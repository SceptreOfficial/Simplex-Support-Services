...
switch (_request) do {
	...
	case 1 : {
		...
		[{!((_this # 0) getVariable "SSS_onTask")},{
			...
			_vehicle setVariable ["SSS_WPDone",false];
			[_entity,_vehicle] call FUNC(clearWaypoints);
			[_vehicle,_position,0,"SAD","COMBAT","RED","FULL","",WP_DONE] call FUNC(addWaypoint);
	...
	case 3 : {
		...
		[{!((_this # 0) getVariable "SSS_onTask")},{
			...
			_vehicle setVariable ["SSS_WPDone",false];
			[_entity,_vehicle] call FUNC(clearWaypoints);
			[_vehicle,_position getPos [_prepDist,_position getDir _vehicle],0,"MOVE","CARELESS","YELLOW","NORMAL","",WP_DONE] call FUNC(addWaypoint);
