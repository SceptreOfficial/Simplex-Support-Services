...
private _ctrl = _display ctrlCreate [QGVAR(ListNBoxMulti),-1,_ctrlGroup];
...
[_ctrl,"LBSelChanged",{
	[{
		...
		private _rowCount = _ctrl getVariable QGVAR(rowCount);
		private _selection = lbSelection _ctrl select {_x < _rowCount};
	...
	},_this] call CBA_fnc_execNextFrame;// lbSelection isn't registered until the next frame
