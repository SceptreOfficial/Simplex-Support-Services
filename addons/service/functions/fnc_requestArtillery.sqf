...
if (isNil {(group _vehicle) getVariable "SSS_protectWaypoints"}) exitWith {[_vehicle,true] remoteExecCall [QEFUNC(common,respawn),2];};
...
[_vehicle,"Rearmed and ready for further tasking."] remoteExecCall [QEFUNC(common,cooldown),2];

[_vehicle,true,_position] call EFUNC(common,updateMarker);

private _ETA = round (_vehicle getArtilleryETA [_position,_magType]);
NOTIFY_2(_vehicle,"Fire mission confirmed. %1 rounds - ETA %2.",_rounds,_ETA call EFUNC(common,properTime))

[{NOTIFY(_this,"Splash - over.")},_vehicle,(_ETA - 5) max 1] call CBA_fnc_waitAndExecute;
[{[_this,false] call EFUNC(common,updateMarker);},_vehicle,_ETA + 10] call CBA_fnc_waitAndExecute;
