...
if (isNil {(group _vehicle) getVariable "SSS_protectWaypoints"}) exitWith {[_vehicle,true] remoteExecCall [QFUNC(respawn),2];};
	...
	case 1 : {
		...
		[{!((_this # 0) getVariable "SSS_onTask")},{
			...
			[{WAIT_CONDITION_WPDONE},{
				...
				[_vehicle,_position,true] call FUNC(transportSmokeSignal);
	...
	case 5 : {
		...
		[{!((_this # 0) getVariable "SSS_onTask")},{
			...
			[{WAIT_CONDITION_WPDONE},{
				...
				[_vehicle,_position,_hoverHeight,_fastrope] call FUNC(transportHover);
				...
				[{
					...
					[_vehicle,false] call EFUNC(common,updateMarker);
