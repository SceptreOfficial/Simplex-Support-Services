...
switch (_service) do {
	case "artillery" : {
		...
		[format ["Fire Mission Parameters - %1",mapGridPosition _position],[
			...
			["SLIDER","Additional units to include with request",[[1,count _nearbyArtillery,0],1],true,{},{2 call EFUNC(CDS,getCurrentValue)}]
		],{
			...
			[_vehicle,_request,_position,_rounds,_dispersion] remoteExecCall [QFUNC(requestArtillery),_vehicle];

			if (_coordinate) then {
				for "_i" from 0 to (_coordinateCount - 1) do {
					private _nearbyVehicle = _nearbyArtillery # _i;
					[_nearbyVehicle,_request,_position,_rounds,_dispersion] remoteExecCall [QFUNC(requestArtillery),_nearbyVehicle];
				};
			};
		},{REQUEST_CANCELLED},[_nearbyArtillery,_vehicle,_request,_position]] call EFUNC(CDS,dialog);
	};
	case "CASDrones" : {
		...
		[_vehicle,_position] remoteExecCall [QFUNC(requestCASDrone),2];
	};
	case "CASGunships" : {
		[_vehicle,_position] remoteExecCall [QFUNC(requestCASGunship),2];
	};
	case "CASHelis" : {
		[_vehicle,_request,_position] remoteExecCall [QFUNC(requestCASHeli),_vehicle];
	};
	case "CASPlanes" : {
		...
		},{REQUEST_CANCELLED},[_vehicle,_request,_position]] call EFUNC(CDS,dialog);
	};
	case "transport" : {
		if (_request isEqualTo 5) then {
			["Hover parameters",[
				...
				[_vehicle,_request,_position,_values] remoteExecCall [QFUNC(requestTransport),_vehicle];
			},{REQUEST_CANCELLED},[_vehicle,_request,_position]] call EFUNC(CDS,dialog);
		} else {
			[_vehicle,_request,_position] remoteExecCall [QFUNC(requestTransport),_vehicle];
		};
	};
};
