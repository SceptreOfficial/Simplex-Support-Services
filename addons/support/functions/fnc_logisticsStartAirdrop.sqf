...
[_entity,_entity getVariable "SSS_cooldownDefault",{LLSTRING(ReadyForNewRequests)}] call EFUNC(common,cooldown);
...
private _ETA = _spawnDelay + ((_spawnPosition distance2D _position) / (getNumber (configFile >> "CfgVehicles" >> _classname >> "maxSpeed") / 3.6));
private _message = {format [LLSTRING(AirdropInboundETA),(_this # 0),[(_this # 1),true] call EFUNC(common,properTime)]};

NOTIFY_2(_entity,_message,_objText,_ETA);
	...
	[{
		...
		[{
			...
			if (_amountLeft <= 0) exitWith {
				...
				private _msg_code = {format [LLSTRING(ObjectDropped),_this # 0]};
