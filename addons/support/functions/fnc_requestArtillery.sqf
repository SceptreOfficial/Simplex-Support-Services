...
[_entity,_cooldownDefault # 0 + (_cooldownDefault # 1 * _rounds),{LLSTRING(RearmedAndReady)}] call EFUNC(common,cooldown);
...
if (_vehicle isKindOf "B_Ship_MRLS_01_base_F") then {
	...
	private _msg_code = {format [LLSTRING(FireMissionConfirmed),_this # 0,_this # 1,PROPER_TIME(_this # 2)]};
	NOTIFY_3(_entity,_msg_code,mapGridPosition _position,_rounds,_ETA);
	[{NOTIFY(_this,{LLSTRING(SplashOver)})},_entity,(_ETA - 5) max 0.5] call CBA_fnc_waitAndExecute;
