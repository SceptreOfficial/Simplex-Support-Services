...
[{
	...
	if (isNull findDisplay IDD_RSCDISPLAYCURATOR) then { 
		private _entity = [
			...
			[LAND_TASK_TYPES,_logic getVariable "TaskTypes"] call EFUNC(common,parseCheckboxes),
		...
		if (alive _vehicle && _vehicle isKindOf "AllVehicles") then {
			[[[_vehicle]]] call FUNC(moduleAddLand_zeus);
		} else {
			FUNC(moduleAddLand_zeus) call EFUNC(common,zeusSelection);
		};
	};
