...
([_player,QSERVICE] call EFUNC(common,getEntities)) apply {
	[[
		...
		{[QSERVICE,_this # 2] call EFUNC(common,openGUI)},{true},{},_x,nil,nil,nil,{
			params ["_target","_player","_entity","_actionData"];

			if ({alive _x} count (_entity getVariable QPVAR(vehicles)) == 0 || _entity getVariable QPVAR(busy)) then {
