...
if (SSS_entities isEqualTo []) exitWith {
	ZEUS_ERROR(localize LSTRING(ZeusNoSupportsExist));
};
...
_title ctrlCommit 0;
_title ctrlSetText localize LSTRING(SelectSupportsToRemove);
...
_cancel ctrlSetText localize LSTRING(CANCEL);
...
_confirm ctrlSetText localize LSTRING(CONFIRM);
[_confirm,"ButtonClick",{
	...
	ZEUS_MESSAGE(localize LSTRING(ZeusSupportsRemoved));
