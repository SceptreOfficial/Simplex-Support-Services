...
[_title,[
	["COMBOBOX","Side",[["BLUFOR","OPFOR","Independent"],0],true,{
		...
		[1,[_displayNames,0]] call EFUNC(CDS,setValues);
	}],
	["COMBOBOX","Callsign",[[],0]]
],{
	...
	((missionNamespace getVariable [format ["SSS_%1_%2",_service,_side],[]]) # _selection) remoteExecCall [QFUNC(remove),2];
},{},_service] call EFUNC(CDS,dialog);
